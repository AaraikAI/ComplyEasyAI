...
|**Cross-Reference Verification** | 2B | Auth chain, IDOR, transaction, API contract, and in-memory state mandatory checks | 
|**Feature Completeness** | 3-4 | Dependency chain tracing, full-stack feature verification, data flow, property-based testing | 
|**Application Logic** | 5 | Business rules, validation, state management, error propagation | | **Security & Red Teaming** | 6-7 | Auth flows, OWASP checks, secrets, API security, data protection, autonomous fuzzing | 
...
Before starting Phase 1 (Build Verification), confirm: - Sufficient disk space for `npm install` + `tsc` (minimum 2GB free) - Node.js and npm are available - All `package.json` files can install dependencies If the build environment **cannot** support compilation: - Score Build & Compile as `N/A` (not 75%, not 0%) - Redistribute its 10% weight across other domains - Document the environment limitation clearly - **Do NOT** let environment issues inflate or deflate the score 
--- 
## PHASE 0: STACK DETECTION & CONTEXT MAPPING 
Before scanning anything, understand what you're auditing. 

#### 0A: Map the Source Tree (NO truncation) ```bash find . -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" -o -name "*.py" -o -name "*.go" -o -name "*.rs" -o -name "*.java" -o -name "*.rb" -o -name "*.php" -o -name "*.vue" -o -name "*.svelte" \) \ | grep -v node_modules | grep -v dist | grep -v build | grep -v __pycache__ | grep -v .venv | grep -v target \ | grep -v ".test." | grep -v ".spec." | grep -v "__tests__" \ | sort > /tmp/audit_all_source.txt wc -l /tmp/audit_all_source.txt cat /tmp/audit_all_source.txt ``` 

#### 0B: Detect Stack & Build Profile 
```bash 
# Dependency files find . -maxdepth 3 \( -name "package.json" -o -name "requirements.txt" -o -name "Pipfile" -o -name "pyproject.toml" -o -name "go.mod" -o -name "Cargo.toml" -o -name "Gemfile" -o -name "composer.json" \) | grep -v node_modules 

# Framework detection for pkg in $(find . -name "package.json" -not -path "*/node_modules/*" -maxdepth 3); do echo "=== $pkg ===" && cat "$pkg" | grep -E '"(react|next|vue|svelte|angular|express|fastify|nest|hono|koa|prisma|drizzle|supabase|firebase|django|flask|fastapi|rails)"' 2>/dev/null done 

# Database, deployment, env, CI/CD find . -name "*.prisma" -o -name "schema.prisma" -o -name "drizzle.config.*" -o -name "knexfile.*" -o -name "*.sql" -o -name "alembic.ini" | grep -v node_modules | sort find . \( -name "Dockerfile*" -o -name "docker-compose*" -o -name "fly.toml" -o -name "vercel.json" -o -name "netlify.toml" -o -name "render.yaml" \) | grep -v node_modules | sort find . -name ".env*" | grep -v node_modules | sort find . -path "*/.github/workflows/*" -o -path "*/.gitlab-ci*" -o -name "Jenkinsfile" 2>/dev/null 
``` 
...
Save ALL results to temp files (no truncation) 
Read 15+ lines of surrounding code for each match 
Classify as: `INTENTIONAL_FEATURE` | `DEV_FALLBACK` | `PRODUCTION_GAP` | `FALSE_POSITIVE` 
...
--- ## PHASE 2B: MANDATORY CROSS-REFERENCE CHECKS 
These checks MUST be performed after completing Phase 2 pattern scans, before proceeding to Phase 3. Every check is required on every run — none are optional. 

### AUTH CHAIN VERIFICATION 
List EVERY `app.get/post/put/patch/delete` and `router.get/post/put/patch/delete` call in `server/src/index.ts` and all route files 
For EACH route, verify the middleware chain includes `authenticate` — or is explicitly documented as public (health, login, register, webhook receiver) 
Any route missing auth middleware that handles user data = `PRODUCTION_GAP` (High) ### IDOR CHECK 
For EVERY endpoint that accepts an ID from `req.body`, `req.params`, or `req.query` — verify the handler validates that ID belongs to the authenticated user/org (not just that the user IS authenticated) 
Special attention to 2FA flows, password reset flows, and any endpoint where the user supplies their own `userId` 

### TRANSACTION CHECK 
6. For EVERY service function that performs 2+ sequential database writes (create/update/delete), verify they are wrapped in `prisma.$transaction()` or equivalent 
7. Pay special attention to billing/payment flows where DB state must match external service state (Stripe, etc.) ### API CONTRACT CHECK 
8. For EVERY frontend API call in `services/api.ts` (or equivalent), verify the path, method, and request body match an actual backend route definition 
9. Mismatches are `PRODUCTION_GAP` (Medium) 

### IN-MEMORY STATE vs REPLICAS CHECK 
10. Find all `new Map()` and `new Set()` in server code 
11. Cross-reference with `docker-compose.prod.yml` replica count 
12. If replicas > 1 and any Map/Set stores user state = `PRODUCTION_GAP` (High) 
--- 
...
``` 
GAP: server/services/billing.ts:45 — calculateInvoice() returns mock data ├── server/controllers/billing.controller.ts:23 — passes result to response ├── server/routes/billing.routes.ts:12 — exposes POST /api/billing/invoice └── frontend/pages/Billing.tsx:67 — displays invoice data FULL STACK AFFECTED: UI → API → Service → (DB missing) 
``` 
--- 
...
**Feature Granularity Rule:** Define one "feature" as ONE user-facing capability accessible via ONE primary route. Sub-features (e.g., Login vs Register vs 2FA) are separate features if they have separate routes or separate API endpoints. List the total feature count and the counting method at the start of Section 3 of the report. Use the SAME granularity every run by anchoring to route definitions (frontend router + backend route files) — not subjective grouping. Discover: frontend routes, backend API endpoints, sidebar/menu items, navigation entries. 
...
#### Visionary Enhancement: Property-Based Test Synthesis For critical business rules discovered in 4B-4C, synthesize property-based tests: - Generate test cases using fast-check (JS) or Hypothesis (Python) that exercise boundary conditions, type coercion edge cases, and invariant violations. - These tests become part of the audit deliverable — the user gets a test suite, not just a report. 
--- 
...
**Auth Bypass Fuzzing**: Synthesize requests with missing/malformed/expired tokens, manipulated JWTs (alg:none, key confusion), and IDOR payloads. Execute against local API endpoints. 
**Injection Payload Testing**: For each unparameterized query found in 6D, craft and execute SQL injection, XSS, and command injection payloads locally. 
**RBAC Escalation Probes**: Simulate requests from lower-privilege roles attempting higher-privilege actions. Verify every endpoint rejects unauthorized access. 
**Rate Limit Validation**: Send burst requests to auth and public endpoints to confirm rate limiting actually triggers. 
**Severity Classification**: Only mark a vulnerability as CRITICAL if the exploit actually succeeds locally. Theoretical risks remain HIGH or MEDIUM. 
...
Migration tested in staging first - Deployment order: Backup → Migrate → Verify → Deploy → Health check 
...
**Database Connection Kill**: Terminate DB connections mid-request. Verify the app returns graceful errors (not crashes or hung connections). Confirm connection pool recovery. 
**Latency Injection**: Add 2-5 second delays to API responses or DB queries. Verify timeouts trigger correctly, loading states render, and no cascading failures occur. 
**Disk Full Simulation**: Fill the temp/log directory. Verify the app handles write failures gracefully without data corruption. 
**Memory Pressure**: Restrict container memory and generate load. Verify OOM handling, graceful degradation, and no data loss. 
**Dependency Outage**: Block outbound calls to third-party services (Stripe, SendGrid, S3). Verify fallback behavior, retry logic, and user-facing error messages. 
**DNS Resolution Failure**: Simulate DNS failures for external services. Verify timeout handling and circuit breaker patterns. 
...
**Page Load States**: Navigate to every route. Capture screenshots. Verify via VLM that pages render correctly (no blank screens, no broken layouts, no placeholder text). 
**Error States**: Trigger API failures and verify error messages render correctly (not raw JSON, not empty screens). 
**Empty States**: Load pages with no data. Verify meaningful empty state messages (not blank tables, not "undefined"). 
**Loading States**: Intercept API responses to delay them. Verify loading indicators appear (not frozen UI). 
**Responsive Breakpoints**: Capture at mobile (375px), tablet (768px), and desktop (1280px). Flag layout breaks. 
**Accessibility Snapshot**: Run axe-core via Playwright. Report WCAG violations. 
...
**Report sections:** 
**Build Status** — Every compile/lint error listed in full 
**AST Semantic Analysis** — Dead code, complexity hotspots, hidden stubs found via AST 
**Production Gaps** — Every gap with file, line, code, impact chain, complete fix instructions 
**Feature Completeness Matrix** — Every feature × every layer with completion % 
**Application Logic Issues** — Business rule gaps, validation holes, state management problems 
**Security Findings & Red Team Results** — Static audit + active exploitation results with severity 
**API Completeness** — Missing validation, inconsistent responses, missing endpoints 
**Deployment Blockers** — Everything that causes failure in production 
**Infrastructure & Chaos Results** — Monitoring, logging, scaling, DR items + resilience test outcomes 
**Visual Verification (VLM)** — Screenshot-backed UI state verification results 
**Final Scorecard** — Weighted readiness across all domains 
**Prioritized Fix List with Executable Patches** — Ordered by severity + dependency, with complexity and full instructions 

### Readiness Scoring 

**Mandatory formula — apply this exact formula for every domain:** 
``` 
Score = 100% - (sum of deductions) 
``` 
**Deduction schedule per finding severity:** 
...
| Medium | -3% | 
| Low | -1% | 
Cap minimum at **0%** (no negative scores) 
If a domain check **COULD NOT BE EXECUTED** (e.g., build failed due to disk space, not code issues) → score it `N/A` and exclude its weight from the total. 
Redistribute weight proportionally across domains that WERE verified. 
**NEVER score an unverified domain** — that inflates or deflates the overall score. 
...
** EVERY finding must include: file path, line number, code snippet, classification, severity. No finding may be reported without all 5. 
**Rule 2 — Self-verification checklist:
** The self-verification step must produce a CHECKLIST, not a yes/no statement. 
Format: 
``` 
Self-Verification Checklist: 
[ ] All app.* and router.* routes checked for auth middleware 
[ ] All req.body.userId patterns checked for IDOR 
[ ] All multi-write service functions checked for transactions 
[ ] All frontend API paths verified against backend routes 
[ ] All in-memory Maps/Sets cross-referenced with replica count 
[ ] All catch blocks checked for empty/swallowed errors 
[ ] All env var defaults checked for production safety 

``` 
If ANY box is unchecked, go perform that check before finalizing. 
**Rule 3 — Scan coverage table:** At the end of every report, include a SCAN COVERAGE section: 

``` 
## SCAN COVERAGE 
| Check Category | Files Checked | Findings | Verified | 
|----------------------------|---------------|----------|----------| 
| Route auth middleware | N routes | N gaps | YES/NO | 
| IDOR on ID params | N endpoints | N gaps | YES/NO | 
| DB transactions | N functions | N gaps | YES/NO | 
| API contract match | N calls | N gaps | YES/NO | 
| In-memory state | N instances | N gaps | YES/NO | 

``` 
Any row with **"NO"** in Verified = report is incomplete. Go back and finish. 
...
A **`fixes/` directory** containing all patches organized by phase and severity. The agent should offer to apply all patches with user approval. 

### Self-Verification Before presenting the report, re-read it and ask: 
**"If I fixed every item here, would there be ANY surprises on a re-scan?”
** If yes, go find what's missing. ONE pass, ZERO surprises.