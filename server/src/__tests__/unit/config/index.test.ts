...
describe('Config Index', () => {
  ...
  describe('validateConfig()', () => {
    ...
    it('should not throw when all required env vars are present and valid', () => {
      ...
      process.env.ENCRYPTION_KEY = 'c'.repeat(16);
    ...
    it('should throw when ENCRYPTION_KEY is too short (< 16 chars)', () => {
      process.env.DATABASE_URL = 'postgresql://test:test@localhost:5432/test';
      process.env.JWT_SECRET = 'a'.repeat(32);
      process.env.JWT_REFRESH_SECRET = 'b'.repeat(32);
      ...
      const { validateConfig } = require('../../../config/index');
      expect(() => validateConfig()).toThrow('ENCRYPTION_KEY must be at least 16 characters');
    });
