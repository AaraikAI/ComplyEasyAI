  ...
  async detectConflicts(
    ...
    try {
      ...
      if (conflicts.length > 0) {
        // Store each conflict as a RegulatoryChange record with changeType 'conflict'
        for (const conflict of conflicts) {
          await prisma.regulatoryChange.create({
            data: {
              organizationId,
              regulationName: `Conflict: ${conflict.regulation1 || 'Unknown'} vs ${conflict.regulation2 || 'Unknown'}`,
              jurisdiction,
              effectiveDate: new Date(),
              changeType: 'conflict',
              affectedFrameworks: conflict.affectedFrameworks || [],
              extractedRequirements: [conflict.description || ''],
              ...
              status: 'pending',
              regulationText: conflict.description,
              source: 'rif_conflict_detection',
              createdBy: 'system',
            },
          });
        }

        // Also log summary to audit trail for traceability
        await prisma.auditLog.create({
          data: {
            ...
            details: JSON.stringify({
              ...
              storedAs: 'RegulatoryChange records with changeType=conflict',
              timestamp: new Date(),
            }),
            userId: 'system',
            organizationId,
...
Return only the resolution text, no JSON or formatting.`;
  ...
  private async monitorWebsiteChanges(
    ...
    try {
      ...
      // Get last known content hash from Feed record
      const lastHash = (feed as any).lastHash || '';

      if (contentHash === lastHash) {
        return []; // No changes
      }

      // Persist the new content hash in the feed's lastHash field
      await prisma.regulatoryFeed.updateMany({
        where: { id: feed.id, organizationId },
        data: { lastHash: contentHash, updatedAt: new Date() },
      });
