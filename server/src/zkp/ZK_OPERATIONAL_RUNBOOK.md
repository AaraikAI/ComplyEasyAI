...
What lives in git and what is regenerated:

- **Verification keys (`keys/verification/*.vkey`) and compiled WASM
  (`compiled/wasm/*.wasm`) are COMMITTED**, together with the build manifest
  (`compiled/build-manifest.json`) and the `.r1cs` constraint files. These are
  small and are needed to verify proofs and reproduce builds, so they are
  tracked normally (re-included via negation rules in the root `.gitignore`).
- **Proving keys (`keys/proving/*.zkey`) are large.** The policy is to keep them
  out of normal git via **Git LFS**
  (`server/src/zkp/keys/proving/*.zkey filter=lfs diff=lfs merge=lfs -text` in
  `.gitattributes`). **If Git LFS is unavailable** in the environment, the
  `.zkey` files are instead **gitignored and regenerated by
  `setup-circuits.sh`** on deploy rather than tracked. (At the time this policy
  was written, `git lfs` was NOT installed, so the gitignore-and-regenerate path
  is in effect.)
- **The powers-of-tau file (`*.ptau`) is NEVER committed.** It is downloaded and
  SHA-verified by `setup-circuits.sh`. Scratch proof I/O
  (`test_input.json`, `proof.json`, `public.json`) is likewise ignored.
- **Runtime proof generation requires the `.zkey` to be present on disk.**
  Therefore every deploy MUST either **fetch the proving keys from Git LFS** (if
  LFS tracking is in use) **or run `setup-circuits.sh`** to regenerate them
  before the ZK proving path is exercised. Verification-only paths need just the
  committed `.vkey`/`.wasm`.
- **CI bakes the proving keys into the production image.** The `docker` job in
  `.github/workflows/ci.yml` ("Docker Build & Push") runs `setup-circuits.sh`
  (after installing the pinned circom + circomlib/snarkjs) to regenerate
  `compiled/` and `keys/` into the build context, and the root `Dockerfile`'s
  `backend-production` stage `COPY`s `server/src/zkp/{compiled,keys}` into
  `dist/zkp/` so the deployed runtime can generate/verify real Groth16 proofs.
