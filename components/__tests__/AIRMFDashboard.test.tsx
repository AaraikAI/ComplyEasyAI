...
vi.mock('@/services/api', () => ({
  api: {
    ...
    aiRmf: {
      ...
      getAISystems: vi.fn().mockResolvedValue([]),
      getSystem: vi.fn().mockResolvedValue(null),
      createSystem: vi.fn().mockResolvedValue({ id: '1' }),
      getAssessments: vi.fn().mockResolvedValue([]),
      getDashboard: vi.fn().mockResolvedValue({ totalSystems: 0, byStatus: {}, byLifecycleStage: {}, byRiskLevel: {}, averageTrustworthinessScore: 0 }),
      getDashboardData: vi.fn().mockResolvedValue({ totalSystems: 3, byStatus: { Deployed: 1, In_Development: 2 }, byLifecycleStage: {}, byRiskLevel: { High: 1, Medium: 2 }, averageTrustworthinessScore: 72 }),
...
describe('AIRMFDashboard', () => {
  ...
  it('calls onNavigate when New AI System button is clicked', async () => {
    render(<AIRMFDashboard onNavigate={mockOnNavigate} />);
    await waitFor(() => {
      expect(screen.getByText('New AI System')).toBeInTheDocument();
    });
    fireEvent.click(screen.getByText('New AI System'));
    expect(mockOnNavigate).toHaveBeenCalledWith('ai-rmf-create');
  });

  it('fetches dashboard data and systems on mount', async () => {
    const { api } = await import('@/services/api');
    render(<AIRMFDashboard onNavigate={mockOnNavigate} />);
    await waitFor(() => {
      expect(api.aiRmf.getDashboardData).toHaveBeenCalled();
