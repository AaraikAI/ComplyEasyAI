...
vi.mock('@/services/api', () => ({
  api: {
    ...
    aiRmf: {
      ...
      getAISystemById: vi.fn().mockResolvedValue({ id: 'sys-1', name: 'Test AI System', status: 'In_Development', systemType: 'Machine Learning', lifecycleStage: 'Plan_and_Design', autonomyLevel: 'Human_in_Loop', trustworthinessCharacteristics: [], coreFunctions: {}, subcategories: [] }),
      ...
      calculateTrustworthinessScore: vi.fn().mockResolvedValue({ score: 80 }),
    },
...
describe('AISystemDetails', () => {
  ...
  it('displays tab navigation options', async () => {
    render(<AISystemDetails systemId="sys-1" onBack={mockOnBack} />);
    await waitFor(() => {
  ...
  it('shows Go Back button when system is not found', async () => {
    const { api } = await import('@/services/api');
