...
vi.mock('@/services/api', () => ({
  api: {
    ...
    enterprise: { getQuestionnaires: vi.fn().mockResolvedValue([]), getPolicies: vi.fn().mockResolvedValue([]), getMonitors: vi.fn().mockResolvedValue([]), getIssues: vi.fn().mockResolvedValue([]), getReports: vi.fn().mockResolvedValue([]), questionnaires: { list: vi.fn().mockResolvedValue([]), getMetrics: vi.fn().mockResolvedValue({ total: 0, byStatus: { draft: 0, inProgress: 0, completed: 0, reviewed: 0, approved: 0 }, aiAssisted: 0, averageAIConfidence: 0, overdue: 0, completionRate: 0, averageCompletionTime: 0 }), getTemplates: vi.fn().mockResolvedValue([]) } },
