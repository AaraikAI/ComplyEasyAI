...
vi.mock('@/services/api', () => ({
  api: {
    ...
    enterprise: { getQuestionnaires: vi.fn().mockResolvedValue([]), getPolicies: vi.fn().mockResolvedValue([]), getMonitors: vi.fn().mockResolvedValue([]), getIssues: vi.fn().mockResolvedValue([]), getReports: vi.fn().mockResolvedValue([]), policies: { list: vi.fn().mockResolvedValue([]), getMetrics: vi.fn().mockResolvedValue({ total: 0, byStatus: { draft: 0, review: 0, approved: 0, archived: 0 }, byCategory: {}, reviewsDue: 0, overdue: 0 }), getTemplates: vi.fn().mockResolvedValue([]) } },
