...
vi.mock('@/services/api', () => ({
  api: {
    ...
    aiRmf: { getSystems: vi.fn().mockResolvedValue([]), getAISystems: vi.fn().mockResolvedValue([]), getSystem: vi.fn().mockResolvedValue(null), createSystem: vi.fn().mockResolvedValue({ id: '1' }), getAssessments: vi.fn().mockResolvedValue([]), getDashboard: vi.fn().mockResolvedValue({ totalSystems: 0, byStatus: {}, byLifecycleStage: {}, byRiskLevel: {}, averageTrustworthinessScore: 0 }), deleteSystem: vi.fn().mockResolvedValue({}), deleteAISystem: vi.fn().mockResolvedValue({}) },
...
describe('AISystemList', () => {
  const mockOnSelectSystem = vi.fn();
  ...
  it('calls onCreateNew when New AI System button is clicked', async () => {
    render(<AISystemList onSelectSystem={mockOnSelectSystem} onCreateNew={mockOnCreateNew} />);
    await waitFor(() => {
