...
vi.mock('@/services/api', () => ({
  api: {
    ...
    acos: { getGoals: vi.fn().mockResolvedValue([]), getControlLoops: vi.fn().mockResolvedValue([]), getEarlyWarnings: vi.fn().mockResolvedValue([]) },
...
describe('ACOSDashboard', () => {
  ...
  it('loads data on mount', async () => {
    ...
    render(<ACOSDashboard onBack={mockOnBack} />);
    await waitFor(() => {
      expect(api.acos.getGoals).toHaveBeenCalled();
      expect(api.acos.getControlLoops).toHaveBeenCalled();
    });
  });
});
