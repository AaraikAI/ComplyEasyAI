...
vi.mock('@/services/api', () => ({ api: { enterprise: { getWorkspaces: vi.fn().mockResolvedValue([]), workspaces: { getHierarchy: vi.fn().mockResolvedValue([]), getConsolidatedMetrics: vi.fn().mockResolvedValue({}), createChild: vi.fn().mockResolvedValue({}), cloneFramework: vi.fn().mockResolvedValue({}), moveUser: vi.fn().mockResolvedValue({}) }, visionaryAI: { getCoPilotRecommendations: vi.fn().mockResolvedValue([]), getBenchmarking: vi.fn().mockResolvedValue({}), predictRisks: vi.fn().mockResolvedValue([]) } } }, getAuthToken: vi.fn().mockReturnValue('token') }));
