...
vi.mock('crypto', () => ({
  default: { createHash: vi.fn().mockReturnValue({ update: vi.fn().mockReturnValue({ digest: vi.fn().mockReturnValue('abc123') }) }) },
  createHash: vi.fn().mockReturnValue({ update: vi.fn().mockReturnValue({ digest: vi.fn().mockReturnValue('abc123') }) }),
}));
...
vi.mock('@/services/api', () => ({
  api: {
    ...
    audit: { list: vi.fn().mockResolvedValue({ logs: [{ id: '1', action: 'Login', user: { name: 'Test User' }, timestamp: new Date().toISOString(), hash: 'abcdef1234567890', organizationId: 'org-1' }], total: 1 }) },
...
describe('AuditTrail', () => {
  ...
  it('displays search/filter controls', async () => {
    render(<AuditTrail />);
    await waitFor(() => {
