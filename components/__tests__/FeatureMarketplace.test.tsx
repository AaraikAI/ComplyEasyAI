...
vi.mock('@/services/api', () => ({
  api: {
    ...
    getAvailableFeatures: vi.fn().mockResolvedValue({ features: [] }),
    getFeatureSubscriptions: vi.fn().mockResolvedValue({ subscriptions: [] }),
    subscribeToFeature: vi.fn().mockResolvedValue({}),
  },
...
describe('FeatureMarketplace', () => {
  ...
  it('calls API to load features on mount', async () => {
    const { api } = await import('@/services/api');
    render(<FeatureMarketplace />);
    await waitFor(() => {
      expect(api.getAvailableFeatures).toHaveBeenCalled();
    });
  });
});
