...
vi.mock('@/services/api', () => ({
  api: {
    ...
    issues: { list: vi.fn().mockResolvedValue([]), getDashboard: vi.fn().mockResolvedValue({ totalIssues: 0, statusDistribution: {}, priorityDistribution: {}, typeDistribution: {}, slaMetrics: {}, overdueIssues: 0, unassignedIssues: 0, averageResolutionTime: 0, issuesByAssignee: [], criticalIssues: [] }) },
  },
...
describe('IssueManagement', () => {
  ...
  it('renders the issue management title', async () => {
    render(<IssueManagement />);
    await waitFor(() => {
      expect(screen.getByText(/Issue Management/i)).toBeTruthy();
    });
  });
  ...
  it('renders an Add Issue / Create button', async () => {
    render(<IssueManagement />);
    await waitFor(() => {
      expect(screen.getByText(/Add Issue|Create Issue|New Issue/i)).toBeTruthy();
    });
  });
