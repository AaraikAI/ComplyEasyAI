...
vi.mock('@/services/api', () => ({ api: { euRegulations: { aiAct: { getSystems: vi.fn().mockResolvedValue({ systems: [] }), registerSystem: vi.fn().mockResolvedValue({}), conductRiskAssessment: vi.fn().mockResolvedValue({}), getTransparencyReports: vi.fn().mockResolvedValue([]), generateTransparencyReport: vi.fn().mockResolvedValue({}) } } }, getAuthToken: vi.fn().mockReturnValue('token') }));

describe('EUAIActDashboard', () => {
  it('should render dashboard', async () => { render(<EUAIActDashboard />); await waitFor(() => { expect(screen.getAllByText(/EU AI Act|AI Act/i).length).toBeGreaterThan(0); }); });
  it('should show loading state initially', () => { render(<EUAIActDashboard />); expect(document.body.innerHTML.length).toBeGreaterThan(0); });
