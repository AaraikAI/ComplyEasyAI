...
describe('AISystemCreate', () => {
  ...
  it('displays required form fields', () => {
    render(<AISystemCreate onBack={mockOnBack} onSuccess={mockOnSuccess} />);
    expect(screen.getByPlaceholderText(/Medical Diagnosis/i)).toBeInTheDocument();
  });
  ...
  it('allows user to type in the name field', () => {
    render(<AISystemCreate onBack={mockOnBack} onSuccess={mockOnSuccess} />);
    const nameInput = screen.getByPlaceholderText(/Medical Diagnosis/i);
    fireEvent.change(nameInput, { target: { value: 'My AI System' } });
    expect(nameInput).toHaveValue('My AI System');
  });
});
