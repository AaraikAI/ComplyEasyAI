...
vi.mock('@/services/api', () => ({
  api: {
    ...
    enterprise: { getQuestionnaires: vi.fn().mockResolvedValue([]), getPolicies: vi.fn().mockResolvedValue([]), getMonitors: vi.fn().mockResolvedValue([]), getIssues: vi.fn().mockResolvedValue([]), getReports: vi.fn().mockResolvedValue([]), monitoring: { list: vi.fn().mockResolvedValue([]), getDashboard: vi.fn().mockResolvedValue({ totalMonitors: 0, activeMonitors: 0, statusDistribution: { passing: 0, warning: 0, failing: 0, unknown: 0 }, typeDistribution: {}, totalAlerts: 0, criticalAlerts: 0, autoRemediatedCount: 0, failingMonitors: [] }), create: vi.fn().mockResolvedValue({}), update: vi.fn().mockResolvedValue({}), delete: vi.fn().mockResolvedValue({}), execute: vi.fn().mockResolvedValue({}), getById: vi.fn().mockResolvedValue({}), toggle: vi.fn().mockResolvedValue({}), getResults: vi.fn().mockResolvedValue([]), aiSuggest: vi.fn().mockResolvedValue([]) } },
...
describe('MonitoringDashboard', () => {
  ...
  it('loads monitors on mount', async () => {
    ...
    render(<MonitoringDashboard />);
    await waitFor(() => {
      expect(api.enterprise.monitoring.list).toHaveBeenCalled();
    });
  });
