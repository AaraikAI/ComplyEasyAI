...
vi.mock('@/services/api', () => ({
  api: {
    ...
    frameworks: { getAll: vi.fn().mockResolvedValue([]), getControls: vi.fn().mockResolvedValue([]), list: vi.fn().mockResolvedValue([]), getDetails: vi.fn().mockResolvedValue({ controls: [], readinessScore: 0 }) },
...
vi.mock('@/hooks/useOnboarding', () => ({
  useOnboarding: vi.fn().mockReturnValue({ isOnboarding: false, currentFlow: null, completeMilestone: vi.fn(), triggerCelebration: vi.fn(), progress: null }),
...
describe('FrameworkDetails', () => {
  ...
  it('renders the framework name', async () => {
    render(<FrameworkDetails framework={mockFramework} onBack={mockOnBack} />);
    expect(screen.getByText('SOC 2')).toBeTruthy();
  });

  it('renders a back button', () => {
    render(<FrameworkDetails framework={mockFramework} onBack={mockOnBack} />);
    const backBtn = screen.getByTestId('icon-ArrowLeft');
  ...
  it('shows loading state initially', () => {
    render(<FrameworkDetails framework={mockFramework} onBack={mockOnBack} />);
    expect(screen.getByText(/loading|SOC 2/i)).toBeTruthy();
  });

  it('handles undefined framework gracefully', () => {
    render(<FrameworkDetails framework={undefined} onBack={mockOnBack} />);
    expect(screen.getByText(/not found|select|no framework/i)).toBeTruthy();
  });
});
