...
describe('OnboardingOverlay', () => {
  ...
  it('renders nothing when not onboarding', () => {
    const { container } = render(<OnboardingOverlay />);
    // When isOnboarding is false, the overlay should not display any visible content
    expect(container.querySelector('[role="dialog"]')).toBeNull();
  });

  it('renders without crashing', () => {
    render(<OnboardingOverlay />);
    // Component should mount without errors even when not active
  });

  it('does not show celebration when showCelebration is false', () => {
    const { container } = render(<OnboardingOverlay />);
